...
public class DownloadReceiver extends BroadcastReceiver {
    ...
    private void handleNotificationBroadcast(Context context, Intent intent) {
        ...
        if (action.equals(Constants.ACTION_OPEN)) {
            Utils.D("Receiver open for " + uri);
        } else if (action.equals(Constants.ACTION_LIST)) {
            Utils.D("Receiver list for " + uri);
        } else { // ACTION_HIDE
            Utils.D("Receiver hide for " + uri);
        }
...
/*        if (destination == Constants.DESTINATION_EXTERNAL && !Utils.isSdcardWritable()) {
            // SDCard 没有挂载，无法进行安装操�?            Utils.makeEventToast(context, context.getString(R.string.warning_sdcard_unmounted),
                    false);
            return;
        }*/
...
/*    private void sendNotificationClickedIntent(Context context, Intent intent, Cursor cursor) {
        context.sendBroadcast(new Intent(com.mappn.gfan.Constants.BROADCAST_CLICK_INTENT));
    }*/
